  ...
  module Runner
    class Base < Actor
        ...
        when Hash
          add_jobs(
            jobs.inject([]) do |j, (name, opts)|
              case opts[:jobs]
              when Hash
                j.concat(opts[:jobs].each_pair { |id, job|
                  { name: name, id: id }.merge!(opts[:general] || {}).deep_merge!(job)
                })
              when Array
      ...
      def has_subscribers?(event)
        subscriptions.has_key?(evnet) and !subscribers(event).empty?
      end
