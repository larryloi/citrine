...
module Citrine
  module Repository
    class Base < Actor
      ...
      def has_models?; !self.class.models.empty?; end
      def migrator_launched?; actor_launched?(:migrator); end
      ...
      def run_migration(action:, **opts)
        wait_for_connection
        _run_migration!(action, **opts)
      rescue StandardError => e
        handle_exception(e)
      end
      ...
      def schema(table)
        ...
        if table_exists?(table)
					Hash[*_schema(table).flatten].inject({}) do |schema, (col, attrs)|
      ...
      def foreign_keys(table)
        if table_exists?(table)
           _foreign_keys(table).inject({}) do |fks, fk|
      ...
      def to_struct(obj); self.class.to_struct(obj); end
      ...
      def connect
        ...
        @connector.pause
        @validator.resume
        create_models if has_models? and !migrator_launched?
        signal :connected
      ...
      def valid_connection?
        connected? and check_connection
      end
